...
echo -e "\n~~ File Handling~~"
    ...
    echo -n "Testing bad filename - "
        timeout 0.2s ./maze fake_maze.txt > out
        ((all_counter++))
...
echo -e "\n~~ Valid Maze Tests~~"
    ...
    echo -n "Testing valid maze (1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_1.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
    ...
    echo -n "Testing valid maze (2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_2.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
        if grep -q "Success : maze loaded successfully" out;
        then
            echo -e "\e[32mPASS\e[0m"
    ...
    echo -n "Testing valid maze (3) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
        if grep -q "Success : maze loaded successfully" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Up Movement Tests~~"
    ...
    echo -n "Testing successful moving up (upper case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_up_upper_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved up" out;
        then
            echo -e "\e[32mPASS\e[0m"
 ...
 # or its the edge
    echo -n "Testing moving into wall upwards - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_blocked_y.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test there are no walls upwards
    echo -n "Testing moving of the edge (up - 1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_start_at_top.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
...
#  # in this test, there are white spaces in top row
    echo -n "Testing moving of the edge (up - 2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_white_space_up.txt < data/moving_up_lower_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: can't move up. That's the edge" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Down Movement Tests~~"
    ...
    echo -n "Testing successful moving down (lower case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_down_lower_case.txt > out
        ((all_counter++))
    ...
    echo -n "Testing successful moving down (upper case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_down_upper_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved down" out;
        then
            echo -e "\e[32mPASS\e[0m"
 ...
 # or its the edge
    echo -n "Testing moving into wall downwards - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_blocked_y.txt < data/moving_down_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test there are no walls downwards
    echo -n "Testing moving of the edge (down - 1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_start_at_bottom.txt < data/moving_down_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test, there are white spaces in bottom row
    echo -n "Testing moving of the edge (down - 2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_white_space_down.txt < data/moving_down_lower_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: can't move down. That's the edge" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Right Movement Tests~~"
    ...
    echo -n "Testing successful moving right (lower case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_right_lower_case.txt > out
        ((all_counter++))
    ...
    echo -n "Testing successful moving right (upper case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_right_upper_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved right" out;
        then
            echo -e "\e[32mPASS\e[0m"
 ...
 # or its the edge
    echo -n "Testing moving into wall right - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_blocked_x.txt < data/moving_right_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test there are no walls right
    echo -n "Testing moving of the edge (right - 1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_start_at_right.txt < data//moving_right_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test, there are white spaces in right column
    echo -n "Testing moving of the edge (right - 2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_white_space_right.txt < data/moving_right_lower_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: can't move right. That's the edge" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Left Movement Tests~~"
    ...
    echo -n "Testing successful moving left (lower case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_left_lower_case.txt > out
        ((all_counter++))
    ...
    echo -n "Testing successful moving left (upper case) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_left_upper_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved left" out;
        then
            echo -e "\e[32mPASS\e[0m"
 ...
 # or its the edge
    echo -n "Testing moving into wall left - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_blocked_x.txt < data/moving_left_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test there are no walls left
    echo -n "Testing moving of the edge (left - 1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_start_at_left.txt < data/moving_left_lower_case.txt > out
        ((all_counter++))
 ...
 # in this test, there are white spaces in left column
    echo -n "Testing moving of the edge (left - 2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_white_space_left.txt < data/moving_left_lower_case.txt > out
        ((all_counter++))
        if grep -q "Dialogue: can't move left. That's the edge" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Bad user inputs ~~"
    ...
    echo -n "Testing bad movement (wrong key) - "
        echo "b" | timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt > out
        ((all_counter++))
    ...
    echo -n "Testing bad movement (empty) - "
        echo | timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt > out
        ((all_counter++))
        if grep -q "Error: invalid key" out;
        then
            echo -e "\e[32mPASS\e[0m"
...
echo -e "\n~~ Testing successful movement ~~"
    echo -n "Testing successful movement (1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_3.txt < data/moving_around_1.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved up" out;
        then
            if grep -q "Dialogue: you moved left" out;
    ...
    echo -n "Testing successful movement (2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_2.txt < data/moving_around_2.txt > out
        ((all_counter++))
        if grep -q "Dialogue: you moved up" out;
        then
            if grep -q "Dialogue: you moved up" out;
...
echo -e "\n~~ Testing Map Opening ~~"
    echo -n "Testing successful map open (1) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_1.txt < data/opening_map_lower_case.txt > out
    ...
    echo -n "Testing successful map open (2) - "
        timeout 0.2s ./maze data/valid_maze/valid_maze_2.txt < data/opening_map_upper_case.txt > out
        # -p enables Perl-compatible regular expressions, allowing multi line grep
        # the -z treats the input as a set of lines, each terminated by a null character.
        ((all_counter++))
        if grep -qPz '########\n#   # ##\n### ## #\n# X ####\n# # ####\n#S#E####' out
...
echo -e "~~ Testing end of game ~~"
    echo -n "Testing successful end of game (1)- "
        timeout 0.2s ./maze data/valid_maze/valid_maze_1.txt < data/moving_to_end_1.txt > out
        ((all_counter++))
    ...
    echo -n "Testing successful end of game (2)- "
        timeout 0.2s ./maze data/valid_maze/valid_maze_2.txt < data/moving_to_end_2.txt > out
        ((all_counter++))
        if grep -q 'You won!' out
        then
            echo -e "\e[32mPASS\e[0m"
